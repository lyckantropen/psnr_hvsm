...
xt::xtensor<double, 1> dct(const xt::xtensor<double, 1> &x, bool norm)
{
...
// 2D DCT-II
xt::xtensor<double, 2> dct2(const xt::xtensor<double, 2> &x, bool norm)
{
  xt::xtensor<double, 2> y = x;
  for (size_t j = 0; j < x.shape(1); ++j)
  {
    xt::row(y, j) = dct(xt::row(y, j), norm);
  }
  for (size_t i = 0; i < x.shape(0); ++i)
  {
    xt::col(y, i) = dct(xt::col(y, i), norm);
  }
  return y;
}
